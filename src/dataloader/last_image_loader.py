...
class lastACDC2cocoConverter(Data2cocoConverter):
    ...
    def iterate_images(self):
        ...
        phase_contr_npzs = []
        segms = []
        self.raw_images_path = self.raw_images_path.replace(
            "acdc_last_images", "acdc_large"
        )
        ...
        for acdc_ds in os.listdir(base_dict):
            ...
            if acdc_ds_name.find(".zip") == -1:
                experiment_dict = os.fsencode(
                    os.path.join(self.raw_images_path, acdc_ds_name)
                )
