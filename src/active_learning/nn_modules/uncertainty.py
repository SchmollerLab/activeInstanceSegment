...
def uncertainty(
    observations, iterations, max_entropy, width, height, device, mode="min"
):
    """
    ...
    for key, val in observations.items():
        ...
        if len(softmaxes[0]) == 1:
            ...
            entropies = torch.stack(
                [
                    torch.distributions.Categorical(softmax).entropy()
                    for softmax in softmaxes
                ]
            )
            entropies_norm = torch.stack(
                [torch.divide(entropy, max_entropy.to(device)) for entropy in entropies]
            ...
            inv_entropies_norm = torch.stack(
                [
                    torch.subtract(torch.ones(1).to(device), entropy_norm)
                    for entropy_norm in entropies_norm
                ]
