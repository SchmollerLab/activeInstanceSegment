...
df.to_csv("uncertainties_vs_num_samples.csv")"""
...
for num_train_data in range(15,570,75):
    cfg_test = cfg
    cfg_test.AL.NUM_MC_SAMPLES
    mc_model = load_model(cfg_test, os.path.join(model_path, f"best_model{str(num_train_data)}.pth"))
    mc_model = patch_module(mc_model)
...
df.to_csv(os.path.join(results_path,"uncertainties_vs_model_progress.csv"))"""
