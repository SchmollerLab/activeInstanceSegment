...
def get_uncertainties(im_json, model, query_strategy, cut=False, mask_iou=True):
...
experiment_name = "mc_drop_aggregation"
    ...
    for mask_iou in [True, False]:
        for agg in ["mean", "max", "sum"]:
            for num_train_data in [15, 240, 3000, 5800]:
                ...
                for im_json in tqdm(sub_samples):
                    ...
                    for run_id in range(1):
                        uncertainties, agg_uncertainty = get_uncertainties(
                            im_json, mc_model, mc_strategy, cut=cut, mask_iou=mask_iou
                        )
                        records.append(
                            {
                                "cut": cut,
                ...
                df.to_csv(os.path.join(results_path, f"mc_drop_agg_{cut}_{mask_iou}_{agg}_{num_train_data}.csv"))
